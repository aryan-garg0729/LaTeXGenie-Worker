...
def json_to_latex(data, column, start_index=0):
    latex_parts = []
    in_references = False
    references = []
    section = False
...
def handler(args):
    ...
    try:
        ...
        try:
            ...
            log.error(f"Error in generate_headers: {e}")
            return
        
        # ---------- core latex and references list
        try:
            latex_output, ref = json_to_latex(miner_data, args.column,main_text_pointer)
        except Exception as e:
            log.error(f"Error in json_to_latex: {e}")
            return
        
        # ------------ reference+grobid ref+llm = final reference list for anystyle->bib
        try:
            ...
            log.error(f"Error in generate_references: {e}")
            return
        ...
        try:
            ...
            latex_output, citation_style = fuzzy_match_citations(
                latex_output, mapping, len(references), threshold=80
            )
